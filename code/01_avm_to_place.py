...
xwalk_block_place["block_2020"] = (
    xwalk_block_place["county"]
    + xwalk_block_place["tract"]
    + xwalk_block_place["block"]
)
xwalk_block_place = xwalk_block_place.drop_duplicates(subset=["block_2020"])

xwalk_block_place["place_2020_id"] = (
    xwalk_block_place["state"] + xwalk_block_place["place"]
)

# xwalk_block_place = xwalk_block_place[xwalk_block_place['state'] != 72]

xwalk_clean = xwalk_block_place[
    ["block_2020", "place_2020_id", "PlaceName", "stab"]
].copy()
...
# get median avms
us_place_avm = (
    us_block_data_place.groupby(["place_2020_id", "PlaceName", "stab"])
    .agg(
        {
...
assert (
    us_place_avm["place_2020_id"].duplicated().sum() == 0
), "Duplicate place_2020_id found in the result"

# drop NAs
us_place_avm.dropna(subset=["avm_2023"], inplace=True)

assert len(us_place_avm[us_place_avm["place_2020_id"] == "2165226"]) == 1
assert len(us_place_avm) > 3e4

us_place_avm.to_csv(
    "data/tidy/us_place_avm.csv", index=False, quoting=csv.QUOTE_NONNUMERIC
)

# save tract AVMs for reference
us_tract_avm = (
    us_block_data.assign(tract_2020_id=us_block_data["block_2020"].str[:11])
    .groupby("tract_2020_id")
    .agg(
        {
            "avm_2023": lambda x: np.nanmedian(x).round(),
            "avg_annual_hpa_2012_2024": "median",
...
us_tract_avm.to_csv(
    "data/tidy/us_tract_avm.csv", index=False, quoting=csv.QUOTE_NONNUMERIC
)
